...
@section('content')
    <div class="section-wrapper">
        <label class="section">Data Sifat Tanah</label>
        ...
        $('table#dataTable tbody').on( 'click', 'td button', function (e) {
            ...
            if($(this).hasClass('delete')) {
                swal({   
                    ...
                    $('.preloader').show();
                    remove(data.id,"{{ route('property.delete') }}", "{{ csrf_token() }}")   
                    .then((result) => {
                        $('.preloader').hide();
                        $.toast({
                            heading: 'Success !!',
        ...
        $("#form").find("input,textarea,select").jqBootstrapValidation(
            {
                ...
                submitSuccess: function($form, event) {
                    ...
                    if($('#type').val() === 'POST')
                    {
                        save(data,"{{ route('property.save') }}", "{{ csrf_token() }}")
                        .then((result) => {
                            $('#form')[0].reset();
                        ...
                        update(data,"{{ route('property.update') }}","{{ csrf_token() }}")
                        .then((result) => {
                            $('#form')[0].reset();
                            $('#modalForm').modal('hide')
                            $('.preloader').hide();
