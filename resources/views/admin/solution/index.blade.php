...
<script>
    $( document ).ready(function() {
        ...
        var dt = $('#dataTable').DataTable({
            ...
            autoWidth: false,
            ajax: {
                    url :"{{ route('property.solution.list') }}",
        ...
        $('table#dataTable tbody').on( 'click', 'td button', function (e) {
            ...
            if($(this).hasClass('delete')) {
                swal({   
                    ...
                    $('.preloader').show();
                    remove(data.id,"{{ route('property.solution.delete') }}", "{{ csrf_token() }}")   
                    .then((result) => {
                        $('.preloader').hide();
                        $.toast({
                            heading: 'Success !!',
        ...
        $("#form").find("input,textarea,select").jqBootstrapValidation(
            {
                ...
                submitSuccess: function($form, event) {
                    ...
                    if($('#type').val() === 'POST')
                    {
                        save(data,"{{ route('property.solution.save') }}", "{{ csrf_token() }}")
                        .then((result) => {
                            $('#form')[0].reset();
                        ...
                        update(data,"{{ route('property.solution.update') }}","{{ csrf_token() }}")
                        .then((result) => {
                            $('#form')[0].reset();
                            $('#modalForm').modal('hide')
                            $('.preloader').hide();
