...
{
    public function __construct(Causes $causes)
    {
        $this->table = $causes;
    }
    ...
    public function save(Request $request)
    {
        try {
            ...
            $validate = Validator::make($request->all(),[
                'name' => 'unique:causes_of_soil_properties,name,null,id,soil_properties_id,'.$request->soil_properties_id
            ]);

            if ($validate->fails())
                return response()->json(['message' => 'Penyebab telah terpakai'], 400);
    ...
    public function update(Request $request)
    {
        try {
            ...
            $validate = Validator::make($request->all(),[
                'name' => 'unique:causes_of_soil_properties,name,'.$request->id.',id,soil_properties_id,'.$request->soil_properties_id
            ]);
